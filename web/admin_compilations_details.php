...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
  ...
  if (isset($_POST) && $_POST) {
    ...
    if ( strlen($name) < 1 ) {
      ...
      if ( $id == null ) {
        $s="insert into compilations (name) values (?)";
        $sth = $dbh->prepare($s,array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
        $sth->bindParam(1, $name, PDO::PARAM_STR);
	if ( $sth->execute() ) {
          $id=$dbh->lastInsertId();
          $msg="New compilation added: ".$id.".";
        } else {
          $msg="Error adding compilation";
        }
      } else {
        $s="update compilations set name=? where id = ?";
        $sth = $dbh->prepare($s,array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
        $sth->bindParam(1, $name, PDO::PARAM_STR);
...
if ($id > 0) {
  $s="select * from compilations where id = ?";
