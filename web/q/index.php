...
switch ($qtype) {
  case "suggestions":
    $sql = "select title from games where title like :query union select name from publishers where name like :query union select name from genres where name like :query union select distinct year from games where year like :query";
    $qvalue = $qvalue.'%';
    break;
  case "publisher":
